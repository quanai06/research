...
import pickle
from src.common.path_utils import get_conv_data_dir, get_root_data_dir, require_data_file

class DBpedia:
    def __init__(self, dataset, debug=False):
        self.debug = debug
        self.dataset_dir = get_root_data_dir(dataset)
        with open(require_data_file(os.path.join(self.dataset_dir, 'dbpedia_subkg.json')), 'r', encoding='utf-8') as f:
            self.entity_kg = json.load(f)
        with open(require_data_file(os.path.join(self.dataset_dir, 'entity2id.json')), 'r', encoding='utf-8') as f:
            self.entity2id = json.load(f)
        with open(require_data_file(os.path.join(self.dataset_dir, 'relation2id.json')), 'r', encoding='utf-8') as f:
            self.relation2id = json.load(f)
        with open(require_data_file(os.path.join(self.dataset_dir, 'item_ids.json')), 'r', encoding='utf-8') as f:
...
class Co_occurrence:
    def __init__(self, dataset,split, entity_max_length, all_items,n_entity,debug=False):
        ...
        input_file = require_data_file(get_root_data_dir(dataset) / 'edge_index_c.pt')
...
class text_sim:
    def __init__(self,pad_entity_id, dataset='inspired'):
        dataset_dir = get_root_data_dir(dataset)
